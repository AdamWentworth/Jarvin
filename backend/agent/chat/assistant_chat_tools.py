...
from backend.agent.chat.chat_intent_patterns import (
    ...
    CALENDAR_DETAILS_RE as _CALENDAR_DETAILS_RE, CALENDAR_LOCATION_RE as _CALENDAR_LOCATION_RE,
    CALENDAR_LOOKUP_RE as _CALENDAR_LOOKUP_RE, CALENDAR_MOVE_RE as _CALENDAR_MOVE_RE,
    CALENDAR_NOTES_RE as _CALENDAR_NOTES_RE, CALENDAR_RENAME_RE as _CALENDAR_RENAME_RE,
    CALENDAR_TITLE_RE as _CALENDAR_TITLE_RE, CANCEL_PATTERNS as _CANCEL_PATTERNS,
    CONFIRM_PATTERNS as _CONFIRM_PATTERNS, GOOGLE_RE as _GOOGLE_RE, LIST_DIR_RE as _LIST_DIR_RE,
    READ_FILE_RE as _READ_FILE_RE, REPO_SEARCH_RE as _REPO_SEARCH_RE, RUN_RE as _RUN_RE,
...
from backend.agent.chat.chat_domain_adapters import (
    ...
    maybe_workspace_tool_response_adapter, safe_weather_tool_response_adapter,
)
...
from backend.agent.chat.chat_tool_helpers import (
    calendar_command_reply as _calendar_command_reply, calendar_create_reply as _calendar_create_reply,
    calendar_delete_request_reply as _calendar_delete_request_reply, calendar_details_reply as _calendar_details_reply,
    calendar_field_update_success_reply as _calendar_field_update_success_reply,
    calendar_lookup_reply as _calendar_lookup_reply, calendar_move_request_reply as _calendar_move_request_reply,
    calendar_update_request_reply as _calendar_update_request_reply, clean_query as _clean_query,
    extract_calendar_create_text as _extract_calendar_create_text, infer_calendar_window_days as _infer_calendar_window_days,
    list_reply as _list_reply, google_search_reply as _google_search_reply,
    normalize_confirmation_text as _normalize_confirmation_text, parse_read_args as _parse_read_args,
    parse_write_args as _parse_write_args, read_file_reply as _read_file_reply, read_reply as _read_reply,
    repo_search_reply as _repo_search_reply, run_reply as _run_reply, web_search_reply as _web_search_reply,
)
...
from backend.agent.chat.chat_host_task_adapters import (
    execute_pending_host_task_adapter,
    maybe_host_task_response_adapter,
)
...
def _help_reply() -> str: return help_reply_impl(tools)
def _finalize_tool_response(response: ToolChatResponse, *, conversation_id: int | None) -> ToolChatResponse:
    return finalize_tool_response_impl(response, conversation_id=conversation_id, remember_active_follow_up_domain=remember_active_follow_up_domain)
def _safe_tool_call(fn: Callable[[], str], fallback: str, *, active_domain: str | None = None) -> ToolChatResponse:
    return safe_tool_call_impl(ToolChatResponse=ToolChatResponse, fn=fn, fallback=fallback, active_domain=active_domain)
...
def _maybe_host_task_response(
    ...
    *,
    ...
    return maybe_host_task_response_adapter(
        text,
        conversation_id=conversation_id,
        client_session_id=client_session_id,
        agent_access_mode=agent_access_mode,
        ...
        build_host_task_payload=build_host_task_payload,
        log_agent_action_event=log_agent_action_event,
        maybe_host_task_response_impl=maybe_host_task_response_impl,
    )
def _maybe_weather_tool_response(text: str, *, conversation_id: int | None) -> ToolChatResponse | None:
    return maybe_weather_tool_response_adapter(text, conversation_id=conversation_id, ToolChatResponse=ToolChatResponse, maybe_handle_weather_request=maybe_handle_weather_request, maybe_weather_tool_response_impl=maybe_weather_tool_response_impl)
...
def _execute_pending_host_task(
    ...
    *,
    ...
    return execute_pending_host_task_adapter(
        pending,
        conversation_id=conversation_id,
        client_session_id=client_session_id,
        ToolChatResponse=ToolChatResponse,
        start_host_task_execution=start_host_task_execution,
        execute_workspace_plan=_execute_workspace_plan,
        update_latest_tool_turn=update_latest_tool_turn,
        log_agent_action_event=log_agent_action_event,
    )
